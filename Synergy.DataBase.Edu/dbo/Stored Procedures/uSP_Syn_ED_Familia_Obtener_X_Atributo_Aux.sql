...
CREATE PROCEDURE [dbo].[uSP_Syn_ED_Familia_Obtener_X_Atributo_Aux]
 /*
 ********************************************************************************************
...
BEGIN
	...
	DECLARE @s_Parametro VARCHAR(20) = 'COLUMNASFAMILIA'
						 ...
						 (
							...
							FROM 
							   (SELECT ' +  @s_sql_columnas + '
								FROM VW_Syn_ED_Familia_Fotocheck_Rep_Atr WHERE PeriodoAcademico = ''' + @as_PeriodoAcademico + 
									''' AND AlumnoGrupo = ''' + @as_AlumnoGrupo + ''' AND Vinculo = ''' + @as_Vinculo + ''' AND DocumentoIdentidad = ''' + @as_DocumentoIdentidad + ''') P
							UNPIVOT
