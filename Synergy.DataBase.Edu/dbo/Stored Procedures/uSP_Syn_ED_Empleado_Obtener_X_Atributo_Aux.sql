...
CREATE PROCEDURE [dbo].[uSP_Syn_ED_Empleado_Obtener_X_Atributo_Aux]
 /*
 ********************************************************************************************
...
BEGIN
	...
	DECLARE @s_Parametro VARCHAR(20) = 'COLUMNASEMPLEADO'
						 ...
						 (
							...
							FROM 
							   (SELECT ' +  @s_sql_columnas + '
								FROM VW_Syn_ED_Empleado_Fotocheck_Rep_Atr WHERE PeriodoAcademico = ''' + @as_PeriodoAcademico + ''' AND Empleado= ''' + @as_CodigoEmpleado + ''') P
							UNPIVOT
