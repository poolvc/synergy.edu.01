...
public partial class proPrincipal : ObjectPage
{

    #region CONSTANTES

    protected const string REPORTE_RUTA = "/Reports/GALEXITO_ALE/repPAE";
    ...
    protected void Listar()
    {
        ...
        //Filtrar
        DataTable dt = ListarProcesoPorUsuario(txtDescripcion.Text.Trim(), UsuarioSistema.IdAplicacion, UsuarioSistema.CodigoUsuario, ddlEstado.SelectedValue, pintPagina);
    ...
    protected void Ejecutar(string pstrFormulario, ListViewItem plviObj)
    {
        ...
        HiddenField hfIdGrupo = (HiddenField)plviObj.FindControl("hfIdGrupo");
        HiddenField hfIdReporte = (HiddenField)plviObj.FindControl("hfIdReporte");

        string strCadena = string.Format("?a=r&iA={0}&iG={1}&iR={2}&f={3}", hfIdAplicacion.Value, hfIdGrupo.Value, hfIdReporte.Value, ObtenerFiltroParametro());
    ...
    protected void Exportar()
    {
        ...
        DataTable dt = ListarProcesoPorUsuario(txtDescripcion.Text.Trim(), UsuarioSistema.IdAplicacion, UsuarioSistema.CodigoUsuario, ddlEstado.SelectedValue, 0);
    ...
    protected DataTable ListarProcesoPorUsuario(string pstrDescripcion, int pinIdAplicacion, string pstrCodigoUsuario, string pstrEstado, int pintPagina)
    {
        ...
        {
            var response = client.ServiceOperation<DataTable>(c => c.ListarPorAplicacion(be).Tables[0]);
            dt = response;
        }
        return dt;
    }
